  ...
  return (
    <div className="max-w-4xl m-auto p-2 mb-5 border dark:border-slate-700">
        ...
        { order.cartList.map((item, index) => (
            <div key={index} className="flex flex-wrap justify-between max-w-4xl m-auto p-2 my-5 ">
                <div className="flex">
                    <Link to={`/products/${item.product.id}`}>
                        <img className="w-32 rounded" src={item.product.poster} alt={item.product.name} />
                    </Link>
                    <div className="">
                        <Link to={`/products/${item.product.id}`}>
                            <p className="text-lg ml-2 dark:text-slate-200">{item.product.name}</p>
                        </Link>
